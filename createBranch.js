...
module.exports = async ({ owner, repo, branch, sha }) => {
  const ref = `refs/heads/${branch}`;
  const octokit = github.getOctokit(process.env.GITHUB_TOKEN);
  try {
    const r = await octokit.rest.repos.getBranch({ owner, repo, branch });
    ...
    if (404 === error.status && "HttpError" === error.name) {
      try {
        const r = await octokit.rest.git.createRef({ owner, repo, sha, ref });
